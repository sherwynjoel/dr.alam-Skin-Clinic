...
function initBookingForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const emailUrl = `mailto:admin@dralamdermcentre.com?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(messageBody)}`;

        // 4. Update Success Interface
        ...
        if (modal) {
            ...
            // Re-bind click events to ensure they trigger fresh every time
            waBtn.onclick = (event) => {
                event.preventDefault();
                window.open(whatsappUrl, '_blank');
                setTimeout(() => { modal.classList.remove('active'); form.reset(); }, 500);
            };
