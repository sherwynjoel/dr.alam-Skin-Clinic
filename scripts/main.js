document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dynamically load Lenis to ensure smooth scrolling on all pages
    ...
    script.onload = () => {
        const lenis = new Lenis({
            ...
            smoothWheel: true,
            smoothTouch: false, // Prevents conflicts on mobile touch screens
            touchMultiplier: 2,
        });
        ...
        // Link internal anchors to Lenis for smoother transitions
    ...
    function showHeroSlide(index) {
        ...
        // Sync with result cards if they exist
        const resultCards = document.querySelectorAll('.hero-result-col .result-arch-card');
        if (resultCards.length > 1) {
            resultCards.forEach(c => c.style.display = 'none');
            // Try to show the card matching the index, or fall back to the first one
            const cardToShow = resultCards[index] || resultCards[0];
    ...
    if (heroSlides.length > 0) {
        ...
        if (heroPrev) heroPrev.addEventListener('click', () => showHeroSlide(heroIndex - 1));
        if (heroNext) heroNext.addEventListener('click', () => showHeroSlide(heroIndex + 1));
